...
function getTodos(){
    ...
    xhr.open("get","http://localhost:8080/mbti_hi/api/todos");
...
function updateTodo(id){
    ...
    xhr.open('PUT','http://localhost:8080/mbti_hi/api/todos');
...
function deleteTodo(id){
    let delTodo = {"id": id};
    let xhr = new XMLHttpRequest();
    xhr.open('DELETE','http://localhost:8080/mbti_hi/api/todos');
    xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
...
function postTodo(todo){
    let xhr = new XMLHttpRequest();
    xhr.open('post','http://localhost:8080/mbti_hi/api/todos');
    xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhr.onload = function(){
...
function todoItemAdd(todoObj){
    console.log(todoObj);
    const li = document.createElement('li');	//<li> 이거랑 같은의미
    li.className = "todo-item";
    //이거 머야? 내가 추가한건데 의미없는거같음 ㅇㅎ
    const textSpan = document.createElement('SPAN');
    textSpan.className = "todo-text" ;

	 
    const todotxt = document.createTextNode(todoObj.todo);
    //const todotxt1 = document.createElement(todoObj.member_id);
    textSpan.appendChild(todotxt);
    //todoSpan.appendChild(todotxt1);
    
    const memberSpan = document.createElement('SPAN');
    memberSpan.className = "member-text";
    let membertxt = "";
    if(todoObj.member != null){
    	membertxt = document.createTextNode(todoObj.member.name);
    }else{
			membertxt = document.createTextNode("몰라!!");
	}
    memberSpan.appendChild(membertxt);
